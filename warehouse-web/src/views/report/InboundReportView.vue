<template>
  <div class="inbound-report-container">
    <!-- Filter -->
    ...
    <el-row :gutter="16" class="summary-row">
      <el-col :span="8">
        <el-card shadow="hover" class="summary-card">
          <div class="summary-label">入库总次数</div>
          <div class="summary-value">{{ summary.totalCount }}</div>
        </el-card>
      </el-col>
      <el-col :span="8">
        <el-card shadow="hover" class="summary-card">
          <div class="summary-label">入库总数量</div>
          <div class="summary-value">{{ summary.totalQuantity }}</div>
        </el-card>
      </el-col>
      <el-col :span="8">
        <el-card shadow="hover" class="summary-card">
          <div class="summary-label">入库总金额</div>
    ...
    <el-card class="table-card" shadow="never">
      <el-table :data="tableData" v-loading="loading" stripe border style="width: 100%">
        ...
        <el-table-column label="入库金额" width="140">
...
import { getInboundReport } from '@/api/report'
...
async function fetchData() {
  loading.value = true
  try {
    const res = await getInboundReport(buildParams())
    const data = res.data
    tableData.value = (data.records ?? data ?? []) as InboundRecord[]
