<template>
  <div class="inbound-audit">
    ...
    <el-dialog
      ...
      :title="dialogReadonly ? '入库单详情' : '审核入库单'"
      width="900px"
      destroy-on-close
    >
...
import { getInboundList, getInboundById, confirmInbound } from '@/api/inbound'
...
async function handleApprove(approved: boolean) {
  ...
  try {
    await ElMessageBox.confirm(`确认${action}该入库单？`, '提示', {
  ...
  auditing.value = true
  try {
    await confirmInbound(currentRow.value.id, {
      approved,
      remark: auditForm.remark,
    })
    ElMessage.success(`入库单已${action}`)
