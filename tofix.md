...
| 优先级 | 模块 | 问题数 | 状态 |
|---|---|---|---|
| **P0** 致命 | 角色/权限分配、借还、盘点、操作日志、仪表盘、入库/出库详情 | 9 | ✅ 全部修复 |
| **P1** 严重 | 全局分页、基础资料筛选、VO 字段填充、用户密码重置 | 7 | ✅ 全部修复 |
| **P2** 中等 | 下拉框 label、审计字段名、JWT/CORS、种子数据 | 4 | ✅ 全部修复 |
| **P3** 低优 | 测试缺失、图表缺失、初始化流程缺失 | 2 | ✅ 全部完成 |
...
P0 → P1 → P2 → P3 顺序已全部完成,每项都按以下流程:
1. 复现 Bug(读源码 + 还原现场)
2. 写失败测试(RED)
3. 修复代码(GREEN)
4. 验证测试通过 + 跑全部回归
5. 更新 tofix.md + git commit

## 全部完成 ✅

**最终测试统计**:
- 后端:47 个 case(原 25 + P2-3 4 + P2-4 3 + P3-1 19 + P3-2 3 − 7 重复)
- 前端:132 个 case(原 126 + P2-1 29 + P2-2 12 + P2-3 4 + P2-4 3 + P3-2 6 − 48 整合)
- **全栈共 179 个测试全部通过** ✅

**Git 提交序列**:
- 2597b73: fix(P2-1) 16 处下拉框 label 错配
- 023f588: fix(P2-2) 入库/出库审计字段错配
- 18d4a85: fix(P2-3) JWT OPTIONS 预检放行 + 补 resetPassword 种子
- 60b5bad: fix(P2-4) 种子数据 parent_id 解耦
- 4c218ee: test(P3-1) 后端核心业务补充 19 个功能测试
- f181407: feat(P3-2) 仪表盘 ECharts 7 天趋势折线图